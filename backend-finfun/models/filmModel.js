...
const Film = {
  ...
  getHargaTicket: (id, callback) => {
    const query = 'SELECT p.id, p.ticket_type, p.price, p.cinema_id, l.venue_name, l.cinema_type, l.film_id FROM ticket_prices p JOIN cinema_locations l ON p.cinema_id = l.id WHERE l.film_id = ?';
    db.query(query, [id], callback);
  },

  getschedule1: (id, callback) => {
    const query = 'SELECT s.id AS schedule_id, s.show_time, p.price, l.venue_name, l.cinema_type, l.film_id FROM schedules s JOIN cinema_locations l ON s.cinema_location_id = l.id JOIN ticket_prices p ON l.id = p.cinema_id WHERE l.film_id = ?';
    db.query(query, [id], callback);
  },
  ...
  getAllBioskop: (id, callback) => {
    db.query('SELECT * FROM films WHERE status_film = ? ORDER BY created_at DESC', [id], callback);
  },
  ...
  addFilmToArtist: (filmId, artistId, pemeran, callback) => {
    const checkQuery = 'SELECT * FROM film_artists WHERE film_id = ? AND artist_id = ?';
    db.query(checkQuery, [filmId, artistId, pemeran], (err, result) => {
      if (err) return callback(err);
      if (result.length > 0) {
        return callback(null);
      }
      const query = 'INSERT INTO film_artists (film_id, artist_id, pemeran) VALUES (?, ?, ?)';
  ...
  buyTicket: (user_id, nama, email, schedule_id, seat_ids, film_id, total_price, callback) => {
    const query = `
      INSERT INTO tickets (user_id, nama, email, schedule_id, seats, film_id, total_price, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
    `;
    const values = [user_id, nama, email, schedule_id, JSON.stringify(seat_ids), film_id, total_price];
  ...
  updatePayTiket: (id,payment, callback) => {
    ...
    const { user_id, nama, email, schedule_id, seat_id, film_id, total_price, payment_id, image } = payment;
    console.log(payment);
    ...
    const query = `
      ...
      WHERE id = ?
    `;
    db.query(query, [user_id, nama, email, schedule_id, JSON.stringify(seat_id), film_id, total_price, payment_id, image, id], callback);
  },
  ...
  create: (film, callback) => {
    const { 
      title, deskripsi, release_year, rating, genre1, genre2, genre3, duration, image, posterImage,   
  ...
  update: (id, film, callback) => {
    const {
      title, deskripsi, release_year, rating, genre1, genre2, genre3,
      duration, image, posterImage, director, status_film,
      netflix_link, appletv_link, hbogo_link, bioskop_link,
      like_user = 0, dislike = 0
    } = film;
