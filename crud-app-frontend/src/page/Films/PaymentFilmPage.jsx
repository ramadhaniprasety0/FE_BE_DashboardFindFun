...
const PaymentFilmPage = () => {
  ...
  const getPaymentData = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/film-payment/${id}/schedule/${tiketId}`);
      ...
      console.error(error);
      Swal.fire("Gagal!", "Terjadi kesalahan saat mengambil data pembayaran.", "error");
    } finally {
      setLoading(false); 
    }
  };
  ...
  function generatePaymentCode(transactionId) {
    ...
    const checksum = (parseInt(randomDigits.slice(0, 5)) + parseInt(randomDigits.slice(5, 10))) % 99; // Simple checksum logic
  ...
  return (
    <div className="payment-page">
      ...
      <Container className="payment-content-container">
        <Row className="p-4 rounded-4 mb-4" style={{backgroundColor: "#4527A0"}}>
          <div className="d-flex gap-5 align-items-center justify-content-between">
            <div className="col-md-4">
              <img src={`http://localhost:3000/${data.image}`} alt="Film" className="rounded-4" style={{ width: "100%" }} />
            </div>
        ...
        <Row>
          <div className="d-flex mb-4">
            <div className="col-md-6">
              ...
              <h1 className="payment-music-title text-dark fw-bold">{data.nama}</h1>
            </div>
            <div className="col-md-6 text-end">
              <p className="payment-label mb-1">EMAIL</p>
              <h1 className="payment-music-title text-dark fw-bold">{data.email}</h1>
            </div>
          </div>
        </Row>
        ...
        <Row>
          <div className="d-flex mb-4">
            <div className="col-md-6">
              ...
              <h1 className="payment-music-title text-dark fw-bold">{formatDate(data.created_at)}</h1>
            </div>
            <div className="col-md-6 text-end">
              <p className="payment-label mb-1">JAM</p>
              <h1 className="payment-music-title text-dark fw-bold">{data.show_time.slice(0, 5)}</h1>
            </div>
          </div>
          <div className="d-flex mb-4">
            <div className="col-md-6">
              <p className="payment-label mb-1">TEMPAT DUDUK</p>
              <h1 className="payment-music-title text-dark fw-bold">{data.seat_ids}</h1>
            </div>
            <div className="col-md-6 text-end">
              <p className="payment-label mb-1">TOTAL TEMPAT DUDUK</p>
              <h1 className="payment-music-title text-dark fw-bold">{data.seat_count}</h1>
            </div>
          </div>
        </Row>
        <Row>
          ...
          <Row className="rounded-4 pt-4" style={{backgroundColor: "#4527A0"}}>
            <Col md={6}>
          ...
          <p className="text-danger text-decoration-underline text-center mb-4 fs-6 mt-4">Tiket yang berhasil terbeli tidak dapat diubah atau dibatalkan</p>
          <p className="text-center mb-4 fs-3" style={{color: "#3D3D6C"}}>Sudah Selesai bayar? Upload bukti disini</p>
          ...
          <Col md={10} lg={12}>
            <div className="instructions-box">
              ...
              <ol className="instructions-list">
                <li>Masukan PIN ATM anda lalu pilih menu <strong>transaksi lainnya</strong></li>
                <li>Pilih menu transfer ke <strong>Virtual akun</strong></li>
                <li>Masukkan kode pembayaran Virtual akun <strong>(a.n FindFun Jaya)</strong></li>
                <li>Cek nominal transfer apakah sudah sesuai dengan total pembayaran pada website</li>
                <li>Jika sudah transfer, <strong>simpan bukti / struk transfer</strong> untuk di upload bukti pembayaran ke website FindFun</li> 
              </ol>
            </div>
          </Col>
        </Row>
