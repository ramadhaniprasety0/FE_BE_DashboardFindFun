...
const DetailFilmComponent = ({ film, actors }) => {
  ...
  // Fungsi untuk mengubah status ditonton
  const toggleWatched = () => {
      ...
      <div className="row detail-film-page d-flex flex-column flex-md-row align-items-start gap-4">
          ...
          <div className="poster-section text-center mb-4 mb-md-0">
            ...
            <Dropdown className="btn-double-group d-flex w-100">
              <Dropdown.Toggle
                variant="success"
                id="dropdown-basic"
                className="btn-film-container btn-film"
              >
                Nikmati Film
              </Dropdown.Toggle>

              <Dropdown.Menu className="dropdown-menu w-100">
                <Dropdown.Item href={film.netflix_link} className="ps-5">
                  <i className="bi bi-tv me-2"></i> Netflix
                </Dropdown.Item>
                <Dropdown.Item href={film.appletv_link} className="ps-5">
                  <i className="bi bi-play-btn me-2"></i>Apple TV
                </Dropdown.Item>
                <Dropdown.Item href={film.hbogo_link} className="ps-5">
                  {" "}
                  <i className="bi bi-film me-2"></i>HBO GO
                </Dropdown.Item>
                <Dropdown.Item
                  href={"/films/tiket/" + film.id + "/bioskop"}
                  target={"_blank"}
                  className="ps-5"
                >
                  <i className="bi bi-ticket-perforated me-2 me-2"></i>Beli
                  Tiket
                </Dropdown.Item>
              </Dropdown.Menu>
              <a
        ...
        <div className="col-auto ms-md-auto">
          <div className="interaction-section text-center">
            {/* Baris Ikon Atas */}
            <div className="icon-row" style={{ cursor: "pointer" }}>
              {/* Ikon Ditonton */}
              <span onClick={toggleWatched}>
                <div className="small">Ditonton</div>
                <i
                  className={`bi fs-4 ${
                    isWatched
                      ? "bi-check-circle-fill active-icon"
                      : "bi-check-circle"
                  }`}
                ></i>
              </span>
              ...
              <span onClick={toggleLike}>
                ...
                <i
                  className={`bi fs-4 ${
                    isLiked ? "bi-heart-fill active-icon" : "bi-heart"
                  }`}
                ></i>
              </span>
            </div>
