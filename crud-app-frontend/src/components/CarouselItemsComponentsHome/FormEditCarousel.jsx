...
const FormUpdateCarousel = () => {
    ...
    const getCarouselData = async () => {
        try {
            const { data } = await axios.get(`http://localhost:3000/api/carousel/${id}`);
    ...
    const handleUpdateCarousel = async (e) => {
        ...
        try {
            setSubmitting(true); 
            const response = await axios.put(`http://localhost:3000/api/carousel/${id}`, formData, {
                headers: {
            ...
            console.error(error);
            Swal.fire('Gagal!', 'Terjadi kesalahan saat memperbarui carousel.', 'error');
        } finally {
            setSubmitting(false);
        }
    };
